...
##############################################################################################################################################################
"""
Evaluate ensemble of models after training.
...
All elemenst inside that list will be used to form the ensemble.
A new folder is created containing the resulting predictions.
...
def ood_ensemble(experiments_models, train_config, config, folders, folder_ensemble, which_split, which_dataset=None):
    ...
    ood_loader = eval_ae.get_setup(config, folders, which_split=which_split, nbr_of_samples_per_class=eval_ae.SAMPLES_PER_CLASS_FOR_OOD[config["dataset"]["name"]], load_model=False, print_dataset=False)

    if train_config["dataset"]["name"].lower() == "sviro_uncertainty":
        test_loader = eval_ae.get_setup(train_config, folders, which_split="test-adults", nbr_of_samples_per_class=eval_ae.SAMPLES_PER_CLASS_FOR_OOD[train_config["dataset"]["name"]], load_model=False, print_dataset=False)
    else:
        test_loader = eval_ae.get_setup(train_config, folders, which_split="test", nbr_of_samples_per_class=eval_ae.SAMPLES_PER_CLASS_FOR_OOD[train_config["dataset"]["name"]], load_model=False, print_dataset=False)
...
def acc_ensemble(experiments_models, train_config, config, folders, folder_ensemble, which_split, which_dataset=None):
    ...
    # get the evaluation data
    eval_loader = eval_ae.get_setup(config, folders, which_split=which_split, nbr_of_samples_per_class=eval_ae.SAMPLES_PER_CLASS_FOR_OOD[config["dataset"]["name"]], load_model=False, print_dataset=False)
...
def ood_ensemble_experiments(ensemble_experiments):
    ...
    for experiment in ensemble_experiments:
        ...
        _, model = eval_ae.get_setup(config, folders, which_split=config["dataset"]["split"], load_model=True, nbr_of_samples_per_class=config["dataset"]["nbr_of_samples_per_class"])
